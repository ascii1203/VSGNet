...
NO_VERBS_WO_EXCLUDE = 24
EXCLUDE=['point', 'run', 'sit', 'smile', 'walk']
...
def class_AP(*args):
    ...
    for k in range(NO_VERBS):
        if VERB2ID[k]: 
            ...
            mean+=AP_s
            if VERB2ID[k] not in EXCLUDE:
                mean_wo_exclude += AP_s
            result.append((VERB2ID[k],AP_s))

    result.append(('Mean',mean/NO_VERBS))
    result.append(('Mean w/o 5 classes', mean_wo_exclude/NO_VERBS_WO_EXCLUDE))
    mean=0.0
    counter=0
